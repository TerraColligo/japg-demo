...
class GeneraLiquidaciones(models.TransientModel):
    ...
    def calculo_create(self):
        ...
        if not employee:
            raise Warning("Please select employee first.")
        ...
        if not contract_id:
            raise Warning("No contract found for Employee %s for given date range."%(employee.name))
        ...
        other_inputs = []
        other_inputs.append((0,0,{'name' :'Prima antiguedad', 'code' : 'PRI', 'contract_id':contract_id, 'amount': self.monto_prima_antiguedad}))
        other_inputs.append((0,0,{'name' :'Indemnizacion', 'code' : 'AGUI', 'contract_id':contract_id, 'amount': self.monto_indemnizacion}))
